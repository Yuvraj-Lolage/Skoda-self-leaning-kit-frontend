    ...
    const fetchLeaderboard = async () => {
      try {
        ...
        const response = await axiosInstance.get(
          "/learning-progress/catalog-all-users",
          {
            headers: {
              Authorization: `Bearer ${localStorage.getItem("token")}`,
            },
        ...
        const mappedUsers: UserProgress[] = usersData.map((user: any) => {
          const totalModules = user.modules?.length || 0;

          const completedModules = user.modules.filter(
            (m: any) => m.completion_percent === 100
          ).length;
          ...
          return {
            ...
            latestScore: overallProgress, // placeholder
